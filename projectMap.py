...
def GetMissionXML():
	...
	return '''<?xml version="1.0" encoding="UTF-8" standalone="no" ?>
			<Mission xmlns="http://ProjectMalmo.microsoft.com" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance">
				...
				<About>
					<Summary>DiamondCollector</Summary>
				</About>
				...
				<AgentSection mode="Creative">
					...
					<AgentHandlers>
						...
						<RewardForTouchingBlockType>''' + \
							"<Block type=\"glass\" reward=\"{}\"/>".format(GLASS_REWARD) + \
						'''
						</RewardForTouchingBlockType>

						<RewardForMissionEnd>
						''' + \
							"<Reward description=\"found_goal\" reward=\"{}\" />".format(GOAL_REWARD) + \
						'''
						</RewardForMissionEnd>
...
def falling_reward(CUR_POS, PREV_POS):
	...
	return JUMP_PENALTY if falldown > 15 else (falldown/3)*EVERY_LEVEL_REWARD
...
def get_action(obs, q_network, epsilon, allow_break_action):
	...
	if np.random.ranf() <= epsilon:
		...
		return np.random.choice([0,1,2,0,0,0,0])
